...
public class ListadoEquiposTableModel implements TableModel,Comparable, Printable {
	...
	private List<ListadoEquiposItem> rows;

	public ListadoEquiposTableModel(){
		rows = new ArrayList<ListadoEquiposItem>();
		listeners = new Vector<TableModelListener>();
	}

	public ListadoEquiposTableModel(List<ListadoEquiposItem> prows){
		this();

		for (ListadoEquiposItem item : prows) {
			rows.add(item);
		}
	}

	public void addRow(){
		rows.add(new ListadoEquiposItem());
	}

	public void addRow(ListadoEquiposItem item){
		rows.add(item);
	}
	 ...
	 */
	public void removeRow(ListadoEquiposItem item){
		rows.remove(item);
	}
	...
	public void addRow(ListadoEquiposItem item, int pos){
		rows.add(pos, item);
	}
	...
	public ListadoEquiposItem getRow(int idx){
		if (idx < 0 || idx >= rows.size())
			return null;

		return (ListadoEquiposItem)rows.get(idx);
	}
	...
	public Object getValueAt(int rowIndex, int columnIndex) {
            ...
            ListadoEquiposItem item = (ListadoEquiposItem) rows.get(rowIndex);
            switch (columnIndex) {
                case COLUMNA_CODIGO:
	...
	public List<ListadoEquiposItem> getRows() {
		return rows;
	}

	public void setRows(List<ListadoEquiposItem> rows) {
		this.rows = rows;
	}
	      ...
	      if (pageIndex == 0)
	      {
	        ...
	        // g.drawString("Hola mundo", 100,100);
	         for (int i=0; i<rows.size();i++) {
	        	 ListadoEquiposItem item = rows.get(i);
